...
FROM (VALUES
    ...
    -- Marketplace takes a 15% commission on the projects rather than charging a fee, so its period
    -- opens at zero. It is priced here anyway because every account gets a subscription, and a plan
    -- with no price is a plan nobody can be onboarded onto.
    ('talent','marketplace','yearly',0)
...
CREATE TABLE tool_submission (
    ...
    approved BOOLEAN,
    published BOOLEAN default false -- whether approving this one got as far as creating its tool row
);
...
-- The published catalogue. A submission is the moderation artefact and stays as it was submitted;
-- approving it copies a row in here, and this is what the details page and the search index read.
-- The unique tool_submission_id is what keeps approval idempotent - approving twice cannot list
-- the same tool twice.
CREATE TABLE tool (
    id BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    tool_submission_id BIGINT NOT NULL UNIQUE REFERENCES tool_submission(id),
    ...
    published BOOLEAN NOT NULL DEFAULT true -- lets a listing be pulled without deleting it
);

-- image_path is copied from the submission as-is, so both rows name the same file under the
-- uploads root and neither owns it.
CREATE TABLE tool_image_metadata (
    tool_id BIGINT NOT NULL REFERENCES tool(id),
    image_path VARCHAR NOT NULL,
    kind VARCHAR NOT NULL REFERENCES image_kind(name),
    display_order INT NOT NULL DEFAULT 0,
    CONSTRAINT tool_image_metadata_unique UNIQUE(tool_id, image_path)
);
